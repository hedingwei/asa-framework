...
public class MemoryMetrics extends AbstractSigarMetric {
    ...
    public static final class ProcMemory {
        ...
        private ProcMemory(long resident, long share, long size) {
            this.resident = resident;
            this.share = share;
            this.size = size;
        }

        public static ProcMemory fromSigarBean(ProcMem mem) {
            return new ProcMemory(mem.getResident(), mem.getShare(), mem.getSize());
        }

        private static ProcMemory undef() {
            return new ProcMemory(-1L, -1L, -1L);
        }
    ...
    public ProcMemory procMem() {
        try {
            return ProcMemory.fromSigarBean(sigar.getProcMem(sigar.getPid()));
        } catch (SigarException e) {
            return ProcMemory.undef();
        }
    }
...
//        registerSwapPagesOut(registry);
        registerProcMemoryResident(registry);
        registerProcMemoryShare(registry);
        registerProcMemorySize(registry);
    }
    ...
    public void registerProcMemoryResident(MetricRegistry registry, String name) {
        registry.register(name, new Gauge<Long>() {
            public Long getValue() {
                return procMem().resident();
            }
        });
    }

    public void registerProcMemoryShare(MetricRegistry registry, String name) {
        registry.register(name, new Gauge<Long>() {
            public Long getValue() {
                return procMem().share();
            }
        });
    }

    public void registerProcMemorySize(MetricRegistry registry, String name) {
        registry.register(name, new Gauge<Long>() {
            public Long getValue() {
                return procMem().size();
            }
    ...
    public void registerProcMemoryResident(MetricRegistry registry) {
        registerProcMemoryResident(registry, MetricRegistry.name(getClass(), "proc-mem-resident"));
    }

    public void registerProcMemoryShare(MetricRegistry registry) {
        registerProcMemoryShare(registry, MetricRegistry.name(getClass(), "proc-mem-share"));
    }

    public void registerProcMemorySize(MetricRegistry registry) {
        registerProcMemorySize(registry, MetricRegistry.name(getClass(), "proc-mem-size"));
    }
